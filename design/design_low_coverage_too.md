...
|-----------|------|-------------|
| `-noCoverage` | Flag | Drop coverage from emission features; use 3D GMM |
| `-useEndMotif` | Flag | Include 5′ end motif score in emission features |
| `-saveModel <path>` | String | Save trained model parameters to file |
| `-loadModel <path>` | String | Load pre-trained model parameters |

When `-noCoverage` and `-useEndMotif` are both set, the emission model is a 3D multivariate mixture of 2 Gaussians per state with features: (fragment length, distance-to-center, motif score).

Note: `-saveModel`/`-loadModel` in `FinaleMe.java` handle HMM parameters (GMM emissions, transitions, initiation). The motif lookup table is handled separately by `CpgFeatureMatrixBuilder` via `-saveMotifLookup`/`-loadMotifLookup`. The FinaleMe model file may optionally also bundle the motif lookup for convenience.

#### 3.2.2 Training Procedure

1. **Input:** High-coverage healthy cfDNA WGS BAM/frag.gz + `.2bit` reference genome (1st positional arg).
...
3. **GMM initialization:** Classify all CpG points into 2 clusters using 3D GMM (max 10,000 iterations). Methylated state identified by larger mean distance-to-center (same heuristic as current).
4. **Baum-Welch:** Full estimation of emissions + transitions + initiation. Fragments with ≥7 CpGs. Max 50 iterations or KL convergence < 1e-4.
5. **Output:** Serialized model file (`-saveModel`) containing:
   - Emission GMM parameters: for each state {m, u}, mixture weights (π), means (μ₁, μ₂), covariances (Σ₁, Σ₂) for 2-component 3D Gaussian mixture.
   - Transition probability matrices: one matrix per inter-CpG distance bin.
   - Initiation probability vectors: one vector per offset-from-fragment-start bin.
   - Motif score lookup table (256 entries).
   - Feature normalization parameters (mean, std for each feature from the training BAM).
...
```
Input:
  - Reference model M_ref (from Step 3.2, loaded via -loadModel)
...
python generate_reference_panel.py \
  --pat-files neutrophil.pat.gz,bcell.pat.gz,tcell.pat.gz,... \
  --cell-type-names Neutrophil,B_cell,T_cell,... \
...
Alternatively, if using pre-computed bigWig methylation files (as in the current TOO pipeline), accept bigWig inputs and convert internally:

```bash
python generate_reference_panel.py \
  --bigwig-files neutrophil.methy.bw,neutrophil.cov.bw,bcell.methy.bw,bcell.cov.bw,... \
  --cell-type-names Neutrophil,B_cell,... \
  --marker-regions cgi_shore_markers.bed \
  --output reference_panel.tsv
```

#### 3.4.6 Compatibility with Existing Panels
...
```
   ...
   -saveMotifLookup motif_lookup.tsv
         ...
         └──► motif_lookup.tsv (256 × 1)
               ...
               ▼
         FinaleMe.java
         -miniDataPoints 7 -gmm -noCoverage -useEndMotif
         -saveModel reference_model.ser
               │
               ▼
         reference_model.ser
   ...
   -loadMotifLookup motif_lookup.tsv
         ...
         ▼
   FinaleMe.java
   -loadModel reference_model.ser
   -adaptEmissionOnly -adaptLambda 0.5 -adaptMaxIter 5
         │
...
**New class members:**
```java
private boolean adaptEmissionOnly = false;
private double adaptLambda = 0.5;
private int adaptMaxIter = 5;
private int adaptMinFragments = 1000;
private HmmModel referenceModel;            // loaded from -loadModel
```
...
Serialize the full model state to a binary file (Java ObjectOutputStream or a custom format):
- Emission GMM parameters per state (means, covariances, mixture weights).
- Transition matrices (one per distance bin).
- Initiation probability vectors.
- Motif score lookup table (from CpgFeatureMatrixBuilder).
- Feature normalization parameters (mean, std per feature from training data).
- Model metadata (training sample ID, feature set, convergence info).
