...
{	
	...
	private List<List<edu.northwestern.epifluidlab.finaleme.utils.ObservationVector>> obsSeqs;
	private ArrayList<HashMap<Integer, Pair<Integer, Double>>> nbCpgDistStates;
	private List<Pair<HashMap<Integer, Pair<Integer, Double>>, List<edu.northwestern.epifluidlab.finaleme.utils.ObservationVector>>> matrixObj;
	 ...
	 *                k-means algorithm}.
	 */
	public KMeansPlusLearner(int nbStates, OpdfMultiMixtureGaussianFactory opdfFactory, List<Pair<HashMap<Integer, Pair<Integer, Double>>, List<edu.northwestern.epifluidlab.finaleme.utils.ObservationVector>>> matrixObj, int nbCpgDist, int features, ArrayList<Integer> mixNumberInFeature, double bayesianFactor,
	...
	{	
		...
		this.obsSeqs = new ArrayList<List<edu.northwestern.epifluidlab.finaleme.utils.ObservationVector>>();
		...
		for( Pair<HashMap<Integer, Pair<Integer, Double>>, List<edu.northwestern.epifluidlab.finaleme.utils.ObservationVector>> row : matrixObj){
			List<edu.northwestern.epifluidlab.finaleme.utils.ObservationVector> tmp = new ArrayList<edu.northwestern.epifluidlab.finaleme.utils.ObservationVector>();
		...
		List<edu.northwestern.epifluidlab.finaleme.utils.ObservationVector> observations = flat(obsSeqs);
	...
	public BayesianNhmmV5<edu.northwestern.epifluidlab.finaleme.utils.ObservationVector> iterate()
	{	
		BayesianNhmmV5<edu.northwestern.epifluidlab.finaleme.utils.ObservationVector> hmm = new BayesianNhmmV5(nbStates,nbCpgDist, (OpdfFactory<? extends Opdf<edu.northwestern.epifluidlab.finaleme.utils.ObservationVector>>) new OpdfMultiMixtureGaussianFactory(features, mixNumberInFeature), bayesianFactor);
	 ...
	 */
	public BayesianNhmmV5<edu.northwestern.epifluidlab.finaleme.utils.ObservationVector> learn()
	{	
		BayesianNhmmV5<edu.northwestern.epifluidlab.finaleme.utils.ObservationVector> hmm = null;
		BayesianNhmmV5<edu.northwestern.epifluidlab.finaleme.utils.ObservationVector> prevHmm = null;
	...
	public int getMethyState(){
		...
		int z = 0;
		for (List<edu.northwestern.epifluidlab.finaleme.utils.ObservationVector> obsSeq : obsSeqs) {
	...
	public int getMethyState(BayesianNhmmV5<edu.northwestern.epifluidlab.finaleme.utils.ObservationVector> hmm){
		int methyState = 0;
	...
	public int getMethyState(BayesianNhmmV5<edu.northwestern.epifluidlab.finaleme.utils.ObservationVector> hmm, boolean notUseMethyPrior){
	...
	//	double bicSmallest = Double.MAX_VALUE;
		...
		//	long number = 0L;
			for (Pair<HashMap<Integer, Pair<Integer, Double>>, List<edu.northwestern.epifluidlab.finaleme.utils.ObservationVector>> obsSeq : matrixObj) {
			...
			//	}
				...
				for (int i = 0; i < states.length; i++) {
					edu.northwestern.epifluidlab.finaleme.utils.ObservationVector o = obsSeq.getSecond().get(i);
					//number++;
	...
	{	
		...
		int z = 0;
		for (List<edu.northwestern.epifluidlab.finaleme.utils.ObservationVector> sequence : obsSeqs){
	...
	private void learnOpdf(BayesianNhmmV5<edu.northwestern.epifluidlab.finaleme.utils.ObservationVector> hmm)
	{
		for (int i = 0; i < hmm.nbStates(); i++) {
			Collection<edu.northwestern.epifluidlab.finaleme.utils.ObservationVector> clusterObservations = clusters.cluster(i);
			
			if (clusterObservations.isEmpty())
				hmm.setOpdf(i, (Opdf<edu.northwestern.epifluidlab.finaleme.utils.ObservationVector>) opdfFactory.factor());
			else
	...
	private boolean optimizeCluster(BayesianNhmmV5<edu.northwestern.epifluidlab.finaleme.utils.ObservationVector> hmm)
	{	
		boolean modif = false;
		int j = 0;
		for (Pair<HashMap<Integer, Pair<Integer, Double>>, List<edu.northwestern.epifluidlab.finaleme.utils.ObservationVector>> obsSeq : matrixObj) {
...
{	
	...
	private Hashtable<edu.northwestern.epifluidlab.finaleme.utils.ObservationVector,ValuePlus> clustersHash;
	//private ArrayList<Collection<ObservationVector>> clusters;
	private ArrayList<Hashtable<edu.northwestern.epifluidlab.finaleme.utils.ObservationVector,Integer>> clusters;
	
	public ClustersPlus(int k, List<edu.northwestern.epifluidlab.finaleme.utils.ObservationVector> observations, MersenneTwister randomEngine)
	{
		
		clustersHash = new Hashtable<edu.northwestern.epifluidlab.finaleme.utils.ObservationVector,ValuePlus>();
		clusters = new ArrayList<Hashtable<edu.northwestern.epifluidlab.finaleme.utils.ObservationVector,Integer>>();

		KMeansPlusPlusClusterer<edu.northwestern.epifluidlab.finaleme.utils.ObservationVector> kmc = new KMeansPlusPlusClusterer<edu.northwestern.epifluidlab.finaleme.utils.ObservationVector>(k, 10000, new EuclideanDistance(), randomEngine);
		
		MultiKMeansPlusPlusClusterer<edu.northwestern.epifluidlab.finaleme.utils.ObservationVector> mkmc = new MultiKMeansPlusPlusClusterer<edu.northwestern.epifluidlab.finaleme.utils.ObservationVector>(kmc, 20);
		List<CentroidCluster<edu.northwestern.epifluidlab.finaleme.utils.ObservationVector>> clusterResults = mkmc.cluster( (Collection<edu.northwestern.epifluidlab.finaleme.utils.ObservationVector>) observations);
		
		for (int i = 0; i < k; i++) {
			Collection<edu.northwestern.epifluidlab.finaleme.utils.ObservationVector> cluster = clusterResults.get(i).getPoints();
			
			Hashtable<edu.northwestern.epifluidlab.finaleme.utils.ObservationVector,Integer> tmp = new Hashtable<edu.northwestern.epifluidlab.finaleme.utils.ObservationVector,Integer>();
			for (edu.northwestern.epifluidlab.finaleme.utils.ObservationVector element : cluster){
	...
	public Collection<edu.northwestern.epifluidlab.finaleme.utils.ObservationVector> cluster(int clusterNb)
	{
	...
	public void put(edu.northwestern.epifluidlab.finaleme.utils.ObservationVector o, int clusterNb)
	{
