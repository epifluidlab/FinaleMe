...
{	
	...
	List<edu.northwestern.epifluidlab.finaleme.utils.ObservationVector> observations;
	 ...
	 *                k-means algorithm}.
	 */
	public GMMLearner(int nbStates, OpdfMultiMixtureGaussianFactory opdfFactory, List<Pair<HashMap<Integer, Pair<Integer, Double>>, List<edu.northwestern.epifluidlab.finaleme.utils.ObservationVector>>> matrixObj, int nbCpgDist, int features, ArrayList<Integer> mixNumberInFeature, double bayesianFactor,
...
{	
	...
	public ArrayList<Hashtable<edu.northwestern.epifluidlab.finaleme.utils.ObservationVector,Double>> clusters;
	public Hashtable<edu.northwestern.epifluidlab.finaleme.utils.ObservationVector,ArrayList<Double>> clusterHash;
	
	public ClustersGMM(int k, List<edu.northwestern.epifluidlab.finaleme.utils.ObservationVector> observations, int feature)
	{
		clusters = new ArrayList<Hashtable<edu.northwestern.epifluidlab.finaleme.utils.ObservationVector,Double>>();
		clusterHash = new Hashtable<edu.northwestern.epifluidlab.finaleme.utils.ObservationVector,ArrayList<Double>>();
		...
		for (int i = 0; i < k; i++) {
			Hashtable<edu.northwestern.epifluidlab.finaleme.utils.ObservationVector,Double> tmp = new Hashtable<edu.northwestern.epifluidlab.finaleme.utils.ObservationVector,Double>();
			clusters.add(tmp);
		}
		
		for (int i = 0; i < observations.size(); i++){
			edu.northwestern.epifluidlab.finaleme.utils.ObservationVector element = observations.get(i);
			double[] row = data[i];
			double sum = 0;
			for (int j = 0; j < k; j++) {
			...
			for (int j = 0; j < k; j++) {
				...
				Hashtable<edu.northwestern.epifluidlab.finaleme.utils.ObservationVector,Double> tmp = clusters.get(j);
				tmp.put(element, v);
				clusters.set(j,tmp);
				list.add(v);
			}
