...
{	
	 ...
	 *                k-means algorithm}.
	 */
	public KMeansPlusLearner(int nbStates, OpdfMultiMixtureGaussianFactory opdfFactory, List<Pair<HashMap<Integer, Pair<Integer, Double>>, List<org.cchmc.epifluidlab.methhmm.utils.ObservationVector>>> matrixObj, int nbCpgDist, int features, ArrayList<Integer> mixNumberInFeature, double bayesianFactor,
	...
	{	
		...
		List<org.cchmc.epifluidlab.methhmm.utils.ObservationVector> observations = flat(obsSeqs);
	...
	public int getMethyState(BayesianNhmmV5<org.cchmc.epifluidlab.methhmm.utils.ObservationVector> hmm, boolean notUseMethyPrior){
	...
	//	double bicSmallest = Double.MAX_VALUE;
		...
		//	long number = 0L;
			for (Pair<HashMap<Integer, Pair<Integer, Double>>, List<org.cchmc.epifluidlab.methhmm.utils.ObservationVector>> obsSeq : matrixObj) {
			...
			//	}
				...
				for (int i = 0; i < states.length; i++) {
					org.cchmc.epifluidlab.methhmm.utils.ObservationVector o = obsSeq.getSecond().get(i);
					//number++;
	...
	{
		for (int i = 0; i < hmm.nbStates(); i++) {
			Collection<org.cchmc.epifluidlab.methhmm.utils.ObservationVector> clusterObservations = clusters.cluster(i);
			
			if (clusterObservations.isEmpty())
				hmm.setOpdf(i, (Opdf<org.cchmc.epifluidlab.methhmm.utils.ObservationVector>) opdfFactory.factor());
			else
	...
	private boolean optimizeCluster(BayesianNhmmV5<org.cchmc.epifluidlab.methhmm.utils.ObservationVector> hmm)
	{	
		boolean modif = false;
		int j = 0;
		for (Pair<HashMap<Integer, Pair<Integer, Double>>, List<org.cchmc.epifluidlab.methhmm.utils.ObservationVector>> obsSeq : matrixObj) {
...
{	
	...
	private Hashtable<org.cchmc.epifluidlab.methhmm.utils.ObservationVector,ValuePlus> clustersHash;
	//private ArrayList<Collection<ObservationVector>> clusters;
	private ArrayList<Hashtable<org.cchmc.epifluidlab.methhmm.utils.ObservationVector,Integer>> clusters;
	
	public ClustersPlus(int k, List<org.cchmc.epifluidlab.methhmm.utils.ObservationVector> observations, MersenneTwister randomEngine)
	{
		
		clustersHash = new Hashtable<org.cchmc.epifluidlab.methhmm.utils.ObservationVector,ValuePlus>();
		clusters = new ArrayList<Hashtable<org.cchmc.epifluidlab.methhmm.utils.ObservationVector,Integer>>();

		KMeansPlusPlusClusterer<org.cchmc.epifluidlab.methhmm.utils.ObservationVector> kmc = new KMeansPlusPlusClusterer<org.cchmc.epifluidlab.methhmm.utils.ObservationVector>(k, 10000, new EuclideanDistance(), randomEngine);
		
		MultiKMeansPlusPlusClusterer<org.cchmc.epifluidlab.methhmm.utils.ObservationVector> mkmc = new MultiKMeansPlusPlusClusterer<org.cchmc.epifluidlab.methhmm.utils.ObservationVector>(kmc, 20);
		List<CentroidCluster<org.cchmc.epifluidlab.methhmm.utils.ObservationVector>> clusterResults = mkmc.cluster( (Collection<org.cchmc.epifluidlab.methhmm.utils.ObservationVector>) observations);
		
		for (int i = 0; i < k; i++) {
			Collection<org.cchmc.epifluidlab.methhmm.utils.ObservationVector> cluster = clusterResults.get(i).getPoints();
			
			Hashtable<org.cchmc.epifluidlab.methhmm.utils.ObservationVector,Integer> tmp = new Hashtable<org.cchmc.epifluidlab.methhmm.utils.ObservationVector,Integer>();
			for (org.cchmc.epifluidlab.methhmm.utils.ObservationVector element : cluster){
	...
	public void put(org.cchmc.epifluidlab.methhmm.utils.ObservationVector o, int clusterNb)
	{
