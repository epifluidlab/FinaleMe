...
- `autosome_1kb_intervals.UCSC.cpgIsland_plus_shore.b37.bed` (tissue-of-origin analysis only): Define the specific genomic regions from which methylation levels will be summarized and compared across samples and reference methylomes. This file can be generated by downloading UCSC.cpgIsland from the UCSC genome browser, filtering for autosomes, and generating 1kb non-overlapped windows
...
```
ls *.cov.b37.bw | perl -ne 'chomp;$cov=$_;$m=$cov;$m=~s/cov/methy_count/;print " -bigWig $m -useMean0 0 -regionMode 0 -bigWig $cov -useMean0 0 -regionMode 0";' >> cfdna.methy_summary.cmd.txt
