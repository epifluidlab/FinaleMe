...
| `-useBaseQ` | (v0.64+) Add per-CpG base-quality score (Phred) as an additional feature dimension, appended at the end of the observation vector. Empirically motivated by [Volkov et al. 2026](https://doi.org/10.64898/2026.03.08.710357), which shows that per-base quality scores at fragment positions correlate with cancer-associated fragmentation features. Complements `-useEndMotif` (which aggregates baseQ-driven errors at the fragment 5' end into a per-motif score) by exposing the per-CpG-site quality variation along the fragment directly. See `-baseQTransform` for the input distribution shape. **Caveat**: meaningful only when baseQ varies per CpG; tabix-fragment input writes a constant baseQ (= `-fragBaseQ` default) and a Pass-1 degeneracy guard will throw if `baseQ.sd == 0`. The training-time flag is recorded in the model's `.meta.tsv` sidecar so subsequent `-decodeModeOnly` / `-aucMode` runs refuse mismatched flags. |
| `-baseQTransform` | (v0.64.1+) How baseQ is transformed before feeding the GMM emission. Choices: **`quantile`** (default; rank-based inverse normal transformation — empirical CDF then `Φ⁻¹`. Makes the marginal exactly N(0,1) regardless of platform Q-score binning, so a model trained on continuous-Phred HiSeq applies cleanly to NovaSeq 4-bin data without retraining), **`raw`** (legacy v0.64.0 behavior: z-score the raw Phred — non-Gaussian on Q-binned platforms but interpretable in Phred units), **`logit`** (z-score `logit(1 − 10^(-Q/10))` — compresses high-Q saturation; close to `raw` at the typical baseQ regime). The transform is recorded in the model sidecar as `baseq_transform`; mismatching it at decode time throws an actionable error. The CDF used by `quantile` is computed per-sample at Pass 1 and not baked into the model — every sample gets its own normalization, which is what makes the trained model platform-portable. |
